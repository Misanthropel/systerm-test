...
parser MyParser(packet_in packet,
    ...
    state parse_ethernet{
        packet.extract(hdr.ethernet);//捕捉帧 
    ...
    state parse_ipv4{ 
        packet.extract(hdr.ipv4);//zhuabao
...
control MyIngress(inout headers hdr,
    ...
    table ipv4_lpm {
        key     = { meta.dst_ipv4 : lpm; }//对目标ip地址进行lpm匹配以执行action存储数据 
...
control MyDeparser(packet_out packet, in headers hdr) {//选择发送顺序 
    apply {
